...
class tempData:
    ...
    def acceptableDiscrepancy(self):
        if (self.max - self.min) / 2 < constants.TEMP_DIFF:
            return True
        else:
            return False
    ...
    def formatData(self, curtainClosed, doorClosed, wakeupstatus):
        ...
        for item in self.time:
            ...
            newData += str(item)
        newData += " "
        newData += wakeupstatus
        return newData

    def displayStatistics(self):
        print("")
        print("Temp: ", self.temp, "   Temp Max: ", self.max, "  Temp margin: +-", ((self.max-self.min)/2), "  Median: ", (self.min + ((self.max-self.min)/2)))
        print("")
