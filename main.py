...
addr = socket.getaddrinfo(IP, 4445)[0][-1]
...
wakeUpForcedStatus = "false"

if wakeUpForced:
    wakeUpForcedStatus = "true"
...
while True:
    ...
    if iterator > constants.ITERATOR_NUM:
        ...
        if not discrepancy_is_low:
            ...
            newData = tempObj.formatData(curtainClosedData, doorClosed, wakeUpForcedStatus)
            # send data to server
            s.connect(addr)
